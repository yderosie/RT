...
/*   Updated: 2015/12/07 19:27:47 by mbarbari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "framework_shape/fk_obj_sphere.h"
#include "framework_shape/fk_obj_ray.h"
...
static t_bool	sphere_crossroad(t_ray ray, t_sphere sphere, t_intersect *i)
{
	...
	if ((vec_radius = vector_dotproduct(etoc, ray.dir)) < 0.)
		return (FALSE);
	t = vector_dotproduct(etoc, etoc) - SQUARE(vec_radius);
	if (t > (radius = SQUARE(sphere.radius)))
		return (FALSE);
	t = sqrt(radius - t);
	t = FT_MIN((vec_radius - t), (vec_radius + t));
	i->pos = vector_sum(ray.pos, vector_scale(ray.dir, t));
	i->v_normal = vector_substract (i->pos, sphere.pos);
	t = vector_dotproduct(i->v_light, i->v_light);
	t = fk_isqrt(t);
